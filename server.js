...
// ══════════════════════════════════════
// CHAMAR CLAUDE (mantido para etapa4)
// ══════════════════════════════════════
...
function buscarNaTexto(texto, busca) {
  ...
  try { const m = new RegExp(escaped).exec(texto); if (m) return m.index; } catch(e) {}
  return -1;
}
...
async function etapa1_lerLivro(filePath) {
  ...
  if (ext === '.pdf') {
    ...
    // Normalizar espaços múltiplos gerados pelo PDF (ex: "Capítulo  4" → "Capítulo 4")
  ...
  // DOCX: extrair HTML preservando tabelas, negritos
...
// ══════════════════════════════════════
// ETAPA 2+3 — DETECTAR ESTRUTURA DIRETA
// Sem chamada à API — extração direta
// do texto para garantir conteúdo sempre
// ══════════════════════════════════════
async function etapa2e3_estruturarLivro(texto) {
  console.log('[2] Detectando estrutura diretamente do texto...');

  // ── Título, subtítulo e autor ──
  const primeiras = texto.substring(0, 600).split('\n').map(l => l.trim()).filter(l => l.length > 2);
  const titulo    = primeiras[0] || '';
  const subtitulo = primeiras[1] || '';

  // Autor: padrão "— Nome Sobrenome" ou "by Nome"
  const autorMatch = texto.match(/—\s*([A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ][a-záéíóúâêîôûãõç]+(?:\s+[A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ][a-záéíóúâêîôûãõç]+)+)/);
  const autor = autorMatch ? autorMatch[1].trim() : '';
  const credencial = '';

  // ── Detectar seções por padrões ──
  const padroes = [
    /^(Prefácio|Prefacio|PREFÁCIO|PREFACIO)$/,
    /^(Apresentação|APRESENTAÇÃO|Apresentacao)$/,
    /^(Introdução|INTRODUÇÃO|Introducao|INTRODUCAO)$/,
    /^(Capítulo|CAPÍTULO|Capitulo|CAPITULO)\s+\d+/,
    /^(Conclusão|CONCLUSÃO|Conclusao|CONCLUSAO)$/,
    /^(Epílogo|EPÍLOGO|Epilogo)$/,
    /^(Posfácio|POSFÁCIO|Posfacio)$/,
    /^(Referências|REFERÊNCIAS|Referencias|Referências Bibliográficas)$/,
  ...
  for (let i = 0; i < linhas.length; i++) {
    ...
    if (!linha || linha.length > 80) continue;        // títulos são curtos
    for (const padrao of padroes) {
      if (padrao.test(linha)) {
        const pos = buscarNaTexto(texto, linha.substring(0, 40));
        if (pos < 0) break;
        // Evitar duplicatas de posição próxima (subtítulo interno ≠ seção)
        const chave = linha.toLowerCase().replace(/\s+/g, ' ');
        if (vistos.has(chave)) break;
        vistos.add(chave);
        secoesBrutas.push({ titulo: linha, pos });
        break;
      }
    }
  }

  // Ordenar por posição real no texto
  secoesBrutas.sort((a, b) => a.pos - b.pos);

  // Remover "Conclusão" que seja subtítulo interno (seguida de outra seção em <3000 chars)
  const secoes = secoesBrutas.filter(function(s, i) {
    if (!/conclus/i.test(s.titulo)) return true;
    const prox = secoesBrutas[i + 1];
    if (prox && (prox.pos - s.pos) < 3000) {
      console.log('[2] Ignorando "' + s.titulo + '" pos ' + s.pos + ' (subtítulo interno)');
      return false;
    }
    return true;
  });

  console.log('[2] Seções detectadas:', secoes.map(s => '"' + s.titulo + '"').join(', '));

  // ── Extrair conteúdo de cada seção ──
  const capitulos = [];

  for (let i = 0; i < secoes.length; i++) {
    const { titulo: tituloSec, pos } = secoes[i];
    const prox = secoes[i + 1] || null;

    const fimTitulo     = texto.indexOf('\n', pos);
    const inicioConteudo = fimTitulo > 0 ? fimTitulo + 1 : pos;
    const fim            = prox ? prox.pos : texto.length;

    const conteudo = texto.substring(inicioConteudo, fim).trim();

    // Quebrar em parágrafos: linha em branco ou linha única curta (subtítulo)
    const blocos = conteudo.split(/\n{2,}/);
    const paragrafos = [];

    for (const bloco of blocos) {
      const t = bloco.trim().replace(/\n/g, ' ');
      if (!t || t.length < 3) continue;

      // Detectar subtítulo: linha curta (< 80 chars) sem ponto final
      const linhasBloco = bloco.trim().split('\n').filter(l => l.trim());
      if (linhasBloco.length === 1 && t.length < 80 && !t.endsWith('.') && !t.endsWith(',')) {
        paragrafos.push({ tipo: 'subtitulo', texto: t, negrito: false });
      } else {
        // Verificar negritos (**texto**)
        const temNegrito = /\*\*[^*]+\*\*/.test(t);
        paragrafos.push({ tipo: 'normal', texto: t, negrito: temNegrito });
      }
    }

    capitulos.push({ numero: String(i + 1), titulo: tituloSec, paragrafos });
    console.log('[2] "' + tituloSec + '" — ' + paragrafos.length + ' parágrafos');
  }

  return { titulo, subtitulo, autor, credencial, capitulos };
}
...
async function etapa5_diagramar(estrutura, pedido, outputPath) {
  ...
  function pCorpo(texto, negrito) {
    if (String(texto).startsWith('[TABELA]')) {
      ...
      return linhas.map(function(linha) {
        return new Paragraph({ alignment:AlignmentType.LEFT, spacing:{before:0,after:60},
  ...
  const totalParas = caps.reduce(function(a,c){return a+(c.paragrafos||[]).length;},0);
  console.log('[5] DOCX gerado:', caps.length, 'seções,', totalParas, 'parágrafos');
}
...
async function executarDiagramacao(pedido) {
  ...
  try {
    ...
    // ETAPAS 2+3: Detectar estrutura diretamente do texto
